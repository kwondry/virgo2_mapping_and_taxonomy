...
# Set VIRGO2DBLoc relative to the script location
VIRGO2DBLoc = os.path.join(SCRIPT_DIR, 'VIRGO2')

# Check if Bowtie2 index files exist
required_index_files = [f"{VIRGO2DBLoc}.{i}.bt2" for i in range(1, 7)]
missing_files = [f for f in required_index_files if not os.path.exists(f)]

if missing_files:
    print("\nError: Bowtie2 index files not found!")
    print("The following required index files are missing:")
    for f in missing_files:
        print(f"  - {os.path.basename(f)}")
    print("\nPlease run the setup script to create the Bowtie2 index:")
    print("  cd resources")
    print("  chmod +x setup_virgo2_db.sh")
    print("  ./setup_virgo2_db.sh")
    print("\nThis will create the necessary index files in the VIRGO2_20250507 directory.")
    exit(1)
