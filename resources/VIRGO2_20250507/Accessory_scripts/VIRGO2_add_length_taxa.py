...
len_data = pd.read_csv("~/bioinformatics_analysis/VIRGO2/12_package/0.VIRGO2.geneLength.txt",sep="\t",header=None)
len_data.columns = ['Gene','Length']
len_data['Gene'] = len_data['Gene'].astype('str')

#taxa annotation, not every gene has designation
taxa_data = pd.read_csv("~/bioinformatics_analysis/VIRGO2/12_package/1.VIRGO2.taxon.txt",sep="\t")
...
gp_data = pd.read_csv("~/bioinformatics_analysis/VIRGO2/12_package/6.VIRGO2.geneProduct.txt",sep="\t")
